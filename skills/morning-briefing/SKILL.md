...
### Step 3: 프로젝트 상태 확인
참조 파일들을 읽어서 현재 프로젝트 상태를 파악한다.

1. `docs/PROJECT_STATUS.md` 읽기 (핵심 수치, 기술스택, 전략)
2. `docs/DIVISION_STATUS.md` 읽기 (15개 Division 상태)
3. 확인할 항목:
   - 147/147 기능 상태 (100% 완료)
   - AGR/MIN 임포트 상태 (완료/진행중)
   - WDC 추출 상태 (완료/진행중)
   - 15개 Division 상태
   - Shopify 심사 상태
   - Product Hunt 승인 상태
...
4. 위 3개 다 없으면 → `docs/NEXT_SESSION_START.md`의 다음 우선순위 작업 추천
