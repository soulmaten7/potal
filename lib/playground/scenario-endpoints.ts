...
const SELLER_ENDPOINTS: EndpointDef[] = [
  {
    id: 'classify',
    name: 'Classify',
    description: 'Classify a product into its HS code using the v3 10-field pipeline — AI 0 calls.',
    method: 'POST',
    ...
    params: [
      ...
      { key: 'material', label: 'Material', type: 'select', required: true, options: MATERIAL_OPTIONS, description: 'Primary material (maps to WCO Section)' },
      { key: 'category', label: 'Category', type: 'select', required: false, options: CATEGORY_OPTIONS, description: 'Category hint for heading selection' },
      { key: 'origin_country', label: 'Origin Country', type: 'select', required: false, options: COUNTRY_OPTIONS, description: 'Manufacturing country' },
      { key: 'destination_country', label: 'Destination Country', type: 'select', required: false, options: COUNTRY_OPTIONS, description: 'For 10-digit national code expansion' },
      { key: 'description', label: 'Description', type: 'string', required: false, placeholder: 'Bifold wallet with coin pocket', description: 'Function or intended use' },
      { key: 'processing', label: 'Processing', type: 'string', required: false, placeholder: 'hand-stitched, dyed', description: 'Manufacturing or finishing method' },
      { key: 'composition', label: 'Composition', type: 'string', required: false, placeholder: '100% full-grain cowhide', description: 'Material composition or blend ratio' },
      { key: 'weight_spec', label: 'Weight / Spec', type: 'string', required: false, placeholder: '120g', description: 'Weight or dimensional spec' },
      { key: 'price', label: 'Price', type: 'number', required: false, placeholder: '45', description: 'Unit price for price-break rules' },
    ],
    exampleResponse: {
      success: true,
      data: {
        ...
        routes: [
          {
            ...
            dutyRate: 0, vatRate: 0.07, vatLabel: 'Sales Tax', dutyRateSource: 'precomputed_mfn', entryType: 'informal',
            deMinimisApplied: false, dutyThresholdUsd: 0,
            hsCode: '610910', hsCodePrecision: 'HS10', hs10Code: '6109100004',
            breakdown: [{ label: 'Product', amount: 28 }, { label: 'Shipping', amount: 5 }, { label: 'Import Duty', amount: 0, note: 'Duty-free' }, { label: 'Sales Tax', amount: 2.31, note: '7.0%' }, { label: 'Processing Fee', amount: 2, note: 'CBP MPF (informal entry)' }, { label: 'Insurance', amount: 0.5, note: '1.5% of CIF' }],
          ...
          },
          {
            ...
            dutyRate: 0, vatRate: 0.19, vatLabel: 'VAT', dutyRateSource: 'precomputed_mfn', entryType: 'formal',
            deMinimisApplied: false, dutyThresholdUsd: 150,
            hsCode: '610910', hsCodePrecision: 'HS10', hs10Code: null,
            breakdown: [{ label: 'Product', amount: 28 }, { label: 'Shipping', amount: 8 }, { label: 'Import Duty', amount: 0, note: 'Duty-free' }, { label: 'VAT', amount: 6.84, note: '19.0%' }, { label: 'Insurance', amount: 0.54, note: '1.5% of CIF' }],
            localCurrency: { totalLandedCost: 40.12, exchangeRate: 0.925, currency: 'EUR' }, ftaApplied: { hasFta: true, ftaName: 'EU-Korea Free Trade Agreement' }, source: 'live',
          },
        ],
...
const IMPORTER_ENDPOINTS: EndpointDef[] = [
  {
    ...
    description: 'Classify industrial/bulk products with the v3 10-field pipeline — maximum accuracy, AI 0 calls.',
    method: 'POST',
    path: '/api/v1/classify',
    params: [
      ...
      { key: 'material', label: 'Material', type: 'select', required: true, options: MATERIAL_OPTIONS, description: 'Primary material (maps to WCO Section)' },
      { key: 'category', label: 'Category', type: 'select', required: false, options: CATEGORY_OPTIONS, description: 'WCO-aligned category hint', defaultValue: 'machinery-pumps' },
      { key: 'origin_country', label: 'Origin Country', type: 'select', required: false, options: COUNTRY_OPTIONS, description: 'Manufacturing country', defaultValue: 'DE' },
      { key: 'destination_country', label: 'Destination Country', type: 'select', required: false, options: COUNTRY_OPTIONS, description: 'For 10-digit national code expansion', defaultValue: 'KR' },
      { key: 'description', label: 'Description', type: 'string', required: false, placeholder: 'Single-stage centrifugal pump for water supply', description: 'Function or intended use' },
      { key: 'processing', label: 'Processing', type: 'string', required: false, placeholder: 'cast iron housing, machined impeller', description: 'Manufacturing or finishing method' },
      { key: 'composition', label: 'Composition', type: 'string', required: false, placeholder: 'cast iron body, stainless steel shaft', description: 'Material composition or components' },
      { key: 'weight_spec', label: 'Weight / Spec', type: 'string', required: false, placeholder: '25kg, 2.2kW motor', description: 'Weight or dimensional spec' },
      { key: 'price', label: 'Price', type: 'number', required: false, placeholder: '850', description: 'Unit price for price-break rules' },
    ],
    exampleResponse: {
      success: true,
