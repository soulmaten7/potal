...
export const POST = withApiAuth(async (req: NextRequest, ctx: ApiAuthContext) => {
  ...
  if (!hsCode || !destination) {
    return apiError(ApiErrorCode.BAD_REQUEST, 'hs_code and destination required.');
  }
  ...
  try {
    ...
    return apiSuccess({
      ...
      alerts: result.alerts.filter(a => a.type === 'new_petition'),
    }, { sellerId: ctx.sellerId });
  } catch (e) {
    return apiError(ApiErrorCode.INTERNAL_ERROR, e instanceof Error ? e.message : 'Safeguard lookup failed.');
  }
});
