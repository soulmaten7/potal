# 다음 세션 시작 가이드
> 마지막 업데이트: 2026-03-14 01:30 KST (Cowork 12 후반 — 44개 MUST 전부 구현 완료, 102/102 MUST 완료, git push 완료)
...
- **44/44 MUST 구현 완료**: P0 12개 + P1 15개 + P2 17개 = 44개, ~45분
- **102/102 MUST 전체 완료**: 기존 58 + 신규 44 = 102개
- **빌드 통과 + git push 완료**
...
### 🔴 P0 — 즉시
1. ~~**KOR AGR 재임포트**~~ ✅ 완료
2. ~~**44개 MUST 미구현 기능 구현**~~ ✅ 전부 완료 (P0 12+P1 15+P2 17=44/44, 2026-03-14)
3. **7개국 HS 10자리 벌크 다운로드 완료 확인** → DB 임포트 + 가격 분기 규칙 추출 (Claude Code 터미널 1 진행중, 외장하드)
4. **WDC 추출 완료 확인** → 2단계 상품명 세분화 → 5억 사전 매핑
5. **240개국 규정 수집 진행 확인** (Claude Code 터미널 2 진행중, 외장하드)

### 🔴 P1 — 이번 주
6. **SHOULD 40개 기능 우선순위 정리 및 구현**: 102 MUST 완료 후 차별화 기능
7. **5억 상품명 사전 매핑 파이프라인**: WDC 전체 상품명 → HS 10자리 룩업 테이블 생성
8. **가격 분기 규칙 테이블**: "valued over/under $X" 조건 코드 추출 → 규칙화
9. **첫 유료 고객 10개 확보 전략 실행**: A그룹(Shopify/WooCommerce) 타겟 영업 시작
