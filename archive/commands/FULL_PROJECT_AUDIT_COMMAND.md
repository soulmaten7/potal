...
```bash
find ~/potal/app/api/v1 -name "route.ts" | sort
```
...
cat ~/potal/app/lib/cost-engine/GlobalCostEngine.ts
# CostEngine.ts
cat ~/potal/app/lib/cost-engine/CostEngine.ts
# macmap-lookup.ts
cat ~/potal/app/lib/cost-engine/macmap-lookup.ts
# trade-remedy-lookup.ts
cat ~/potal/app/lib/cost-engine/trade-remedy-lookup.ts
# section301-lookup.ts
cat ~/potal/app/lib/cost-engine/section301-lookup.ts
# eu-vat-rates.ts
cat ~/potal/app/lib/cost-engine/eu-vat-rates.ts
# ioss-oss.ts
cat ~/potal/app/lib/cost-engine/ioss-oss.ts
# origin-detection.ts
cat ~/potal/app/lib/cost-engine/origin-detection.ts
# index.ts
cat ~/potal/app/lib/cost-engine/index.ts
# types.ts
cat ~/potal/app/lib/cost-engine/types.ts
# adapters.ts
cat ~/potal/app/lib/cost-engine/adapters.ts
```

**2-3. AI Classifier (app/lib/cost-engine/ai-classifier/)**
```bash
# 전체 파일 읽기
cat ~/potal/app/lib/cost-engine/ai-classifier/claude-classifier.ts
cat ~/potal/app/lib/cost-engine/ai-classifier/vector-search.ts
cat ~/potal/app/lib/cost-engine/ai-classifier/product-mappings.ts
cat ~/potal/app/lib/cost-engine/ai-classifier/ai-classifier-wrapper.ts
cat ~/potal/app/lib/cost-engine/ai-classifier/confidence-score.ts
cat ~/potal/app/lib/cost-engine/ai-classifier/audit-trail.ts
cat ~/potal/app/lib/cost-engine/ai-classifier/description-validator.ts
cat ~/potal/app/lib/cost-engine/ai-classifier/index.ts
```

**2-4. HS Code 관련 (app/lib/cost-engine/hs-code/)**
```bash
ls -la ~/potal/app/lib/cost-engine/hs-code/
# 안의 모든 .ts 파일 전부 읽기
find ~/potal/app/lib/cost-engine/hs-code -name "*.ts" -exec cat {} \;
```

**2-5. DB 레이어 (app/lib/cost-engine/db/)**
```bash
ls -la ~/potal/app/lib/cost-engine/db/
# 안의 모든 .ts 파일 전부 읽기
find ~/potal/app/lib/cost-engine/db -name "*.ts" -exec cat {} \;
```

**2-6. Tariff API (app/lib/cost-engine/tariff-api/)**
```bash
ls -la ~/potal/app/lib/cost-engine/tariff-api/
find ~/potal/app/lib/cost-engine/tariff-api -name "*.ts" -exec cat {} \;
```

**2-7. Screening & Restrictions**
```bash
find ~/potal/app/lib/cost-engine/screening -name "*.ts" -exec cat {} \;
find ~/potal/app/lib/cost-engine/restrictions -name "*.ts" -exec cat {} \;
```

**2-8. Monitoring & Notifications**
```bash
find ~/potal/app/lib/monitoring -name "*.ts" -exec cat {} \;
find ~/potal/app/lib/notifications -name "*.ts" -exec cat {} \;
```

**2-9. API Auth & Billing**
```bash
find ~/potal/app/lib/api-auth -name "*.ts" -exec cat {} \;
find ~/potal/app/lib/billing -name "*.ts" -exec cat {} \;
```

**2-10. 페이지 컴포넌트 핵심**
```bash
cat ~/potal/app/page.tsx
cat ~/potal/app/layout.tsx
cat ~/potal/app/dashboard/DashboardContent.tsx
cat ~/potal/app/pricing/page.tsx
cat ~/potal/app/developers/page.tsx
cat ~/potal/components/layout/Header.tsx
cat ~/potal/components/layout/Footer.tsx
```

**2-11. Middleware & Config**
```bash
cat ~/potal/middleware.ts
cat ~/potal/vercel.json
cat ~/potal/next.config.ts
cat ~/potal/package.json
cat ~/potal/.env.local
```

**2-12. MCP Server**
```bash
cat ~/potal/mcp-server/src/index.ts 2>/dev/null || find ~/potal/mcp-server/src -name "*.ts" -exec cat {} \;
cat ~/potal/mcp-server/package.json
cat ~/potal/mcp-server/server.json
cat ~/potal/mcp-server/registry-metadata.json
```

**2-13. Shopify Extension**
```bash
cat ~/potal/extensions/potal-widget/shopify.extension.toml
find ~/potal/extensions/potal-widget -name "*.liquid" -exec echo "--- {} ---" \; -exec cat {} \;
```

**2-14. Plugins (WooCommerce, BigCommerce, Magento)**
```bash
cat ~/potal/plugins/woocommerce/potal-landed-cost/potal-landed-cost.php
cat ~/potal/plugins/bigcommerce/potal-widget-installer.js
find ~/potal/plugins/magento -name "*.php" -exec echo "--- {} ---" \; -exec cat {} \;
```

**2-15. AI Agents**
```bash
cat ~/potal/ai-agents/custom-gpt/gpt-instructions.md
cat ~/potal/ai-agents/custom-gpt/openapi-gpt-actions.json
cat ~/potal/ai-agents/gemini-gem/gem-instructions.md
cat ~/potal/ai-agents/meta-ai/ai-studio-instructions.md
cat ~/potal/ai-agents/B2B_OUTREACH_TARGETS.md
cat ~/potal/ai-agents/LLM_COMMERCE_INTEGRATION_ANALYSIS.md
```

**2-16. Supabase Migrations 전부**
```bash
# 파일명 목록
ls -la ~/potal/supabase/migrations/
# 017 이후 마이그레이션만 읽기 (007~009는 너무 큼, 스킵)
for f in ~/potal/supabase/migrations/01[0-9]*.sql ~/potal/supabase/migrations/02*.sql ~/potal/supabase/migrations/03*.sql; do
  echo "=== $(basename $f) ==="
  cat "$f"
  echo ""
done
...
```bash
# Python 스크립트
find ~/potal/scripts -name "*.py" -exec echo "=== {} ===" \; -exec cat {} \;
# TypeScript 스크립트
find ~/potal/scripts -name "*.ts" -exec echo "=== {} ===" \; -exec cat {} \;
```

**2-18. Tests**
```bash
find ~/potal/__tests__ -name "*.ts" -exec echo "=== {} ===" \; -exec cat {} \;
```

**2-19. Cron Jobs (app/api/cron/)**
```bash
find ~/potal/app/api/cron -name "route.ts" -exec echo "=== {} ===" \; -exec cat {} \;
```
...
```bash
cd ~/potal && npm run build 2>&1 | tail -30
```
