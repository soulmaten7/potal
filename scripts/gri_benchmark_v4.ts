/**
 * GRI v4.0 Broker+ConflictEngine Benchmark
...
import { classifyWithGRI_v4 } from '../app/lib/cost-engine/gri-classifier/pipeline-v4';
...
async function runBenchmark() {
  ...
  for (let i = 0; i < testItems.length; i++) {
    ...
    try {
      const result = await classifyWithGRI_v4({
        productName,
  ...
  // Build summary
  const summary = `
GRI v4.0 Broker+ConflictEngine Benchmark — ${new Date().toISOString().split('T')[0]}
${'━'.repeat(50)}
