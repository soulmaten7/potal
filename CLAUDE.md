...
16. **데이터 관련 작업 전 기존 상태 확인 필수 (CW38 신설)** — "데이터가 없다", "데이터가 부족하다"는 판단을 내리기 전에 **반드시** 아래 3개를 먼저 확인: (a) `docs/EXTERNAL_DRIVE_FILES.md` — 외장하드에 뭐가 있는지, (b) `docs/DATA_SOURCE_AUDIT_REPORT.md` — Supabase에 뭐가 들어있는지, (c) 실제 Supabase 테이블 row count 확인. "없다"고 말하려면 위 3개를 확인한 증거를 먼저 제시. **확인 안 하고 "데이터 수집 필요"라고 제안하는 것은 금지.** 근거: CW38에서 이미 MacMap 2.4억 건 + VAT/De Minimis 241개국이 Supabase에 있었는데, 확인 안 하고 27개국 웹서치 수집을 진행해서 시간 낭비 발생.
