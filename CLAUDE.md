...
## ⚠️ 로깅 규칙 (최우선 — Claude Code & Cowork 공통)
> **이 규칙은 다른 모든 작업보다 우선한다. 작업을 시작하기 전에 반드시 읽고 따를 것.**

### Claude Code 엑셀 로깅
모든 작업을 `POTAL_Claude_Code_Work_Log.xlsx`에 기록한다:
- **시트 규칙**: 새 작업 시작 시 새 시트 생성. 시트 이름 = `YYMMDDHHMM`
- **열 구조**: A:순번 | B:시간(HH:MM:SS KST) | C:구분 | D:상세내용 | E:파일경로 | F:상태
- **구분**: COMMAND / RESULT / ANALYSIS / DECISION / ERROR / FIX
- **상태**: ✅성공 / ❌실패 / ⏳진행중 / 🔄수정
- **디테일**: COMMAND=명령어 그대로, RESULT=전체 출력, ERROR=전문+스택트레이스, FIX=변경 전/후
- **시트 마감**: `=== 작업 종료 === | 소요시간 | 빌드 | 테스트 | 생성/수정파일`
- Python openpyxl 사용
...
│   └── meta-ai/             # Meta AI (폐기 — 미국 전용, 사용 불가)
│
...
- **관세율 자동업데이트**: Vercel Cron **24개** (CW15 후반: +7 데이터 유지보수 + CW18 5차: +1 data-management + CW18 6차: +1 api-key-monitor weekly Mon 07:00)
...
- **Custom LLM 2종 리라이트**: GPT Actions(API연동, B2B CTA), Gemini Gem(정적데이터+CTA). Meta AI는 미국 전용이라 폐기
- **LLM 통합 전략**: MCP 서버 1개 = Google UCP(MCP 내장) + Anthropic Claude + Shopify UCP 3개 생태계 동시 진입. 분석: ai-agents/LLM_COMMERCE_INTEGRATION_ANALYSIS.md
...
- **API 엔드포인트**: 103개 → **~155개+** (CW14 후반: +45 S+ 업그레이드 + CW18 6차: +7 webhooks CRUD/deliveries, keys/rotate, api-key-monitor)
...
- **DB 마이그레이션 6개+**: 023~024(분류/가격분기) + 039~042(API Key 보안/웹훅시스템/IP규칙/웹훅보안) 외 다수
...
- **Layer 2 GRI Pipeline 프로덕션 배포**: ✅ (CW18 6차) gri-classifier/ 25개 파일 Vercel 배포 성공, /api/v1/classify 프로덕션 테스트 3종 PASS
- **Sprint 1 보안 6기능 100%**: ✅ (CW18 6차) F006(Confidence Score), F012(HS Validation), F046(Webhook System), F052(API Auth), F093(Webhook Security), F125(API Key Security) — 95 unit tests ALL PASS
- **홈페이지 UI 업데이트**: ✅ (CW18 6차) 6개 공개 페이지 20건 수정 — 9-field 100% HS Accuracy 반영, tariff lines 131K, ~155 endpoints
...
### 📂 과거 세션 히스토리 (별도 파일)
> **CW13~CW18 세션별 상세 성과는 `docs/sessions/COWORK_SESSION_HISTORY.md`에 분리됨.**
> 필요 시 Read 도구로 해당 파일을 읽어서 참조할 것.
> 포함 내용: CW13-CW18 Cowork 세션 성과, v3 파이프라인 디테일, Layer 구조, 벤치마크 히스토리, 생성/수정 파일 목록 등
...
11. **로깅 필수** — 상단 "⚠️ 로깅 규칙" 섹션 참조. Claude Code는 엑셀, Cowork는 대화 로그 파일에 반드시 기록
